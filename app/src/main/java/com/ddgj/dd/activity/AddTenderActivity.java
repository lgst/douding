...
public class AddTenderActivity extends BaseActivity implements NetWorkInterface, View.OnClickListener {
    ...
    private void commit() {
        ...
        if (!mImagePath.isEmpty()) {
            ...
            for (String path : mImagePath) {
                ...
                String s = "invite_t_picture";
                post.addFile(s + i++, file.getName(), file);
            }
        }
        final SweetAlertDialog dialog = showLoadingDialog("正在发送您的招标信息！", "");
        HashMap<String, String> params = new HashMap<>();
        params.put("invite_t_title", mTitle.getText().toString().trim());
        params.put("invite_t_price", mPrice.getText().toString().trim());
        params.put("invite_t_start_time", "");
        params.put("invite_t_cycle", mTime.getText().toString().trim());
        params.put("invite_t_pattern", (String) mTenderMod.getSelectedItem());
        params.put("invite_t_require", mMissionInfo.getText().toString().trim());
        params.put("invite_t_phone", mPhoneNumber.getText().toString().trim());
        params.put("invite_t_u_name", mUserName.getText().toString().trim());
        params.put("invite_t_u_id", user.getAccount_id());
        params.put("invite_t_type", String.valueOf(mTenderMod.getSelectedItem()));
        post.url(ADD_TENDER).params(params).build().readTimeOut(10000).execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
                loge("招标发布出错：" + e.getMessage());
                showToastNotNetWork();
                dialog.dismiss();
            }
            ...
            public void onResponse(String response, int id) {
                ...
                if (responseInfo.getStatus() == 0) {
                    showToastShort("招标发布成功！");
                    finish();
                } else {
                    showToastShort(responseInfo.getMsg());
                }
                dialog.dismiss();
            }
        });
    ...
    private boolean submit() {
        ...
        if (TextUtils.isEmpty(timeString)) {
            Toast.makeText(this, "招标周期不能为空", Toast.LENGTH_SHORT).show();
            mTime.requestFocus();
            return false;
        }
