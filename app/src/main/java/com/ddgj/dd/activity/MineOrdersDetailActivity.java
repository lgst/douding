...
public class MineOrdersDetailActivity extends BaseActivity implements NetWorkInterface, AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    private void initData() {
        OkHttpUtils.get().url(GET_ORDER_USERS + "?made_id=" + getIntent().getStringExtra("id")).build().execute(new StringCallback() {
            ...
            public void onResponse(String response, int id) {
                Log.e(TAG, "我的订制详情onResponse: " + response);
                try {
                    ...
                    if (jo.getInt("status") == 0) {
                        ...
                        if (jo.getString("sum").equals("null")) {
//                            setData();
                            return;
                        }
                        ...
                        for (int i = 0; i < ja.length(); i++) {
                            if (new JSONObject(ja.getString(i)).getString("account_type").equals("0")) {
                                ...
                                Log.e("lgst", "gongchang：" + user.getAccount_id());
                                mUsers.add(user);
                            } else {
                                EnterpriseUser user = new Gson().fromJson(ja.getString(i), EnterpriseUser.class);
                                Log.e("lgst", "putong：" + user.getAccount_id());
                                mUsers.add(user);
                            }
                        }
//                        setData();
    ...
    private void regectCheck() {
        ...
        params.put("o_c_u_id", user.getAccount_id());
...
//        Log.i(TAG, "ocuid: "+mOrder.getMade_o_u_id());
    ...
    class ImageGVAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            Glide.with(MineOrdersDetailActivity.this).load(mImagesList.get(position))
                    .into(iv);
            iv.setLayoutParams(new AbsListView.LayoutParams(
                    AbsListView.LayoutParams.MATCH_PARENT,
                    DensityUtil.dip2px(MineOrdersDetailActivity.this, 100)));
            return iv;
        }
    }
    ...
    class LvAdapter extends BaseAdapter {
        ...
        private void confirm(int position) {
            ...
            params.put("made_id", mOrder.getMade_id());
            params.put("o_c_u_id", mUsers.get(position).getAccount_id());
            Log.e("lgst", "接受订单出错：" + mUsers.get(0).getAccount_id() + mOrder.getMade_id() + mUsers.size() + mUsers.get(position).getAccount());
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            Glide.with(MineOrdersDetailActivity.this)
                    .load(HOST + "/" + url)
                    .into(vh.mCivIcon);
            ...
            vh.mBtnCancel.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    new SweetAlertDialog(MineOrdersDetailActivity.this, SweetAlertDialog.WARNING_TYPE)
                            .setConfirmText("确认")
                            .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                                @Override
                                public void onClick(SweetAlertDialog sweetAlertDialog) {
        ...
        private void moreClick(View v, final User user) {
            ...
            switch (v.getId()) {
                case R.id.btn_more:
                    PopupMenu popup = new PopupMenu(MineOrdersDetailActivity.this, v);
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            switch (item.getItemId()) {
                                case R.id.send_message://发送消息
                                    startActivity(new Intent(MineOrdersDetailActivity.this, ChatActivity.class)
